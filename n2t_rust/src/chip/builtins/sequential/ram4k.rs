...
/// RAM4K - 4096-register RAM using 12-bit address
#[derive(Debug)]
...
impl Ram4kChip {
    pub fn new() -> Self {
        ...
        input_pins.insert("address".to_string(), Rc::new(RefCell::new(Bus::new("address".to_string(), 12))) as Rc<RefCell<dyn Pin>>);
        output_pins.insert("out".to_string(), Rc::new(RefCell::new(Bus::new("out".to_string(), 16))) as Rc<RefCell<dyn Pin>>);
        
        Self {
            name: "RAM4K".to_string(),
            input_pins,
            output_pins,
            internal_pins: HashMap::new(),
            clock_subscriber: None,
            memory: Memory::new(4096), // 2^12 = 4096 registers
            next_data: 0,
            current_address: 0,
        }
    }
...
impl ClockedChip for Ram4kChip {
    fn tick(&mut self, _clock_level: Voltage) -> Result<()> {
        ...
        let load = self.input_pins["load"].borrow().voltage(None)?;
        let address = self.input_pins["address"].borrow().bus_voltage() as usize;
        self.current_address = address & 0b111111111111; // Mask to 12 bits for RAM4K
...
mod tests {
    ...
    fn test_ram4k_basic_structure() {
        ...
        // Test memory size
        assert_eq!(ram4k.memory().size(), 4096);
    }
    ...
    fn test_ram4k_address_masking() {
        ...
        // Test that addresses are properly masked to 12 bits
        // Address 4096 (0b1000000000000) should be masked to 0 (0b000000000000)
        ...
        // Check that value was written to address 0 (4096 & 0b111111111111 = 0)
        ram4k.get_pin("address").unwrap().borrow_mut().set_bus_voltage(0);
        ram4k.get_pin("load").unwrap().borrow_mut().pull(LOW, None).unwrap();
        ram4k.eval().unwrap();
        let output = ram4k.get_pin("out").unwrap().borrow().bus_voltage();
    ...
    fn test_ram4k_boundary_addresses() {
        ...
        let test_addresses = [0, 1, 255, 256, 511, 512, 1023, 1024, 2047, 2048, 4095];
